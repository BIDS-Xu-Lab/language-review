...
function onClickAcceptHuman() {
    ...
    store.msg(`Updated human decision [${store.working_item.decision}] to paper ${store.working_item.note_id}`, 'success');
}
...
async function onClickAccept(model_id, result) {
    ...
    store.msg(`Updated [${model_id}] decision [${store.working_item.decision}] to paper ${store.working_item.note_id}`, 'success');
}
